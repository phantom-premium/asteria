...
function handleWSMessage(user, conn, msg) {
  ...
  if (msg.type === 'group-call-join') {
    ...
    // В канале трансляцию может ЗАПУСТИТЬ только владелец/админ сайта — остальные
    // подписчики могут лишь присоединиться к уже идущей трансляции.
    if (isFirst && conv.type === 'channel' && conv.ownerId !== user.id && !user.isAdmin) return;
