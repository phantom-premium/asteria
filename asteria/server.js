...
function userConversations(userId) {
  return db.all('conversations').filter((c) => {
    ...
    if (c.type === 'channel') return c.participants.includes(userId) || c.ownerId === userId;
    return false;
  });
}
...
function broadcastToConversation(conv, payload, exceptUserId) {
  const targets = conv.type === 'channel' ? [...(conv.participants || []), conv.ownerId] : conv.participants;
...
function handleWSMessage(user, conn, msg) {
  ...
  if (msg.type === 'message') {
    ...
    if (!isMember) return;
    if (conv.type === 'channel' && conv.ownerId !== user.id && !conv.everyoneCanPost) return;
  ...
  if (msg.type === 'group-call-join') {
    ...
    if (!isMember) return;
    if (!conv.everyoneCanPost || conv.groupCallsEnabled === false) return;
...
async function handleAPI(req, res, pathname, query = {}) {
  ...
  if (pathname === '/api/me' && method === 'PATCH') {
    const body = await readBody(req);
    const patch = {};
    ['displayName', 'status', 'avatar', 'theme'].forEach((k) => {
  ...
  if (subMatch && method === 'POST') {
    ...
    if (!conv || conv.type !== 'channel') return sendJSON(res, 404, { error: 'Канал не найден' });
  ...
  if (unsubMatch && method === 'POST') {
    ...
    if (!conv || conv.type !== 'channel') return sendJSON(res, 404, { error: 'Канал не найден' });
    if (conv.ownerId === user.id) return sendJSON(res, 400, { error: 'Владелец не может отписаться — удалите канал' });
  ...
  if (convItemMatch && method === 'PATCH') {
    ...
    if (!conv) return sendJSON(res, 404, { error: 'Не найдено' });
    if (conv.type !== 'channel') return sendJSON(res, 400, { error: 'Редактировать можно только каналы' });
    if (conv.ownerId !== user.id && !user.isAdmin) return sendJSON(res, 403, { error: 'Недостаточно прав' });
    const body = await readBody(req);
    const patch = {};
    ['name', 'avatar', 'everyoneCanPost', 'groupCallsEnabled'].forEach((k) => { if (body[k] !== undefined) patch[k] = body[k]; });
  ...
  if (convItemMatch && method === 'DELETE') {
    const conv = db.findById('conversations', convItemMatch[1]);
    if (!conv) return sendJSON(res, 404, { error: 'Не найдено' });
    const isChannelOwner = conv.type === 'channel' && conv.ownerId === user.id;
    if (!isChannelOwner && !user.isAdmin) return sendJSON(res, 403, { error: 'Недостаточно прав' });
  ...
  // Список публичных каналов (для поиска/подписки)
  ...
  if (pathname === '/api/upload' && method === 'POST') {
    ...
    const subdir = kind === 'avatar' ? 'avatars' : kind === 'story' ? 'stories' : '';
  ...
  if (msgItemMatch && method === 'DELETE') {
    ...
    const conv = db.findById('conversations', message.conversationId);
    if (conv) broadcastToConversation(conv, { type: 'message-delete', messageId: message.id, conversationId: message.conversationId }, null);
    return sendJSON(res, 200, { ok: true });
  }
  ...
  if (adminUserMatch && method === 'DELETE') {
    ...
    db.all('conversations').forEach((conv) => {
      const inChannelAsMember = conv.type === 'channel' && (conv.participants || []).includes(target.id) && conv.ownerId !== target.id;
      const shouldDeleteWhole = (conv.type === 'dm' && conv.participants.includes(target.id)) ||
        (conv.type === 'channel' && conv.ownerId === target.id);
      if (shouldDeleteWhole) {
        broadcastToConversation(conv, { type: 'conversation-deleted', conversationId: conv.id }, null);
        db.remove('conversations', conv.id);
        db.removeManyBy('messages', 'conversationId', conv.id);
        removeConvFromAllFolders(conv.id);
      } else if (inChannelAsMember) {
  ...
  if (pathname === '/api/calls' && method === 'GET') {
    const myConvIds = new Set(userConversations(user.id).filter((c) => c.type === 'channel').map((c) => c.id));
