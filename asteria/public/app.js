...
function handleWSEvent(msg) {
  if (msg.type === 'message') {
    ...
    if (state.activeConvId === msg.conversationId) { updateGroupCallButton(conv); renderPinnedBar(conv); }
...
function renderPinnedBar(conv) {
  ...
  const standalone = $('#channelBroadcastBar');
  const pinned = conv.pinnedMessage;
  const live = conv.type === 'channel' && (conv.groupCallCount || 0) > 0;

  if (!pinned) {
    bar.classList.add('hidden');
    bar.classList.remove('pinned-message-bar--live');
    bar.onclick = null;
    const oldRow = bar.querySelector('.pinned-live-row');
    ...
    bar.classList.toggle('pinned-message-bar--live', live);
    bar.onclick = (e) => {
      if (e.target.closest('#unpinMessageBtn') || e.target.closest('.pinned-live-join')) return;
      scrollToMessage(pinned.id);
    };
    let liveRow = bar.querySelector('.pinned-live-row');
    if (live) {
      if (!liveRow) {
        liveRow = document.createElement('div');
        liveRow.className = 'pinned-live-row';
        liveRow.innerHTML = `<span class="live-dot"></span><span class="pinned-live-text">Идёт трансляция</span><button class="btn-secondary pinned-live-join">Присоединиться</button>`;
        bar.appendChild(liveRow);
      }
      liveRow.querySelector('.pinned-live-join').onclick = (e) => {
        e.stopPropagation();
        const c = state.conversations.find((x) => x.id === conv.id);
        if (c) startOrJoinGroupCall(c);
      };
    } else if (liveRow) {
      liveRow.remove();
    }
  ...
  // Трансляция без закреплённого сообщения — отдельным баром
  if (live && !pinned) {
    standalone.classList.remove('hidden');
    $('#channelBroadcastJoinBtn').onclick = () => {
      const c = state.conversations.find((x) => x.id === conv.id);
      if (c) startOrJoinGroupCall(c);
    };
  } else {
...
function updateGroupCallButton(conv) {
  ...
  if (!conv || (conv.type !== 'group' && conv.type !== 'channel') || conv.groupCallsEnabled === false) {
    btn.classList.add('hidden');
    return;
  }
  const canBroadcast = conv.type === 'group' || conv.ownerId === state.user.id || state.user.isAdmin;
  const count = conv.groupCallCount || 0;
  // В каналах кнопку в шапке видит только владелец/админ (запуск трансляции);
  // остальные подписчики присоединяются через баннер "Идёт трансляция".
  if (!canBroadcast && !(conv.type === 'channel' && count > 0)) {
    btn.classList.add('hidden');
    return;
  }
  ...
  if (conv.type === 'channel') {
    btn.title = count > 0 ? `Трансляция идёт (сейчас ${count})` : 'Начать трансляцию';
  } else {
...
async function startOrJoinGroupCall(conv) {
  ...
  $('#groupCallTitle').textContent = conv.type === 'channel' ? `Трансляция · ${conv.name}` : conv.name;
