...
async function openChannelModal(convId) {
  ...
  html += `
    <div class="modal-subtitle">Локальное имя (только у вас)</div>
    <input type="text" id="channelEditNickname" value="${escapeHtml((conv.peerNickname) || '')}" placeholder="Например: Рабочий чат">`;
  ...
  const nickEl = $('#channelEditNickname');
  if (nickEl) nickEl.addEventListener('change', async () => {
    const { conversation } = await api(`/api/conversations/${convId}/nickname`, { method: 'POST', body: { nickname: nickEl.value.trim() } });
    mergeConversation(conversation);
  });
...
async function handleDeepLinkIfPresent() {
  ...
  if (inviteMatch) {
    ...
    try {
      ...
      if (exact) openProfileModal(exact.id);
