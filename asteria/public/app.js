...
function renderConvList() {
  ...
  visibleConversations().forEach((conv) => {
    ...
    meta.innerHTML = `<div class="conv-name"><span>${pinned ? '📌 ' : ''}${escapeHtml(title)}${conv.type==='channel' ? ' 📢' : ''}</span></div><div class="conv-last">${escapeHtml(preview)}</div>`;
...
function renderChatHeader(conv) {
  ...
  $('#chatHeaderInfo').innerHTML = `<div class="avatar">${initials(title)}</div><div><div class="chat-header-name">${escapeHtml(title)}</div><div class="chat-header-sub">${escapeHtml(sub)}</div></div>`;
...
async function openProfileModal(userId) {
  ...
  el.innerHTML = `
    ...
    <div class="profile-name">${escapeHtml(user.displayName)}${user.isBot ? ' 🤖' : ''}</div>
...
function renderMessageBubble(m) {
  ...
  let inner = '';
  const senderName = m.senderId === state.user.id ? '' : `<div class="sender">${escapeHtml(senderLabel(m.senderId))}</div>`;
...
function renderUserSearchResults(users) {
  ...
  users.forEach((u) => {
    ...
    row.innerHTML = `<div class="avatar">${initials(u.displayName)}</div><div><div>${escapeHtml(u.displayName)}</div><div class="chat-header-sub">@${escapeHtml(u.username)}</div></div>`;
...
async function renderAdminUsers() {
  ...
  users.forEach((u) => {
    ...
    row.innerHTML = `
      ...
      <div class="grow">
        <div class="name">${escapeHtml(u.displayName)} ${u.isAdmin ? '<span class="admin-badge">admin</span>' : ''} ${u.isBot ? '🤖' : ''}</div>
