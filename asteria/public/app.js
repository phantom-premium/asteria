...
async function openChannelModal(convId) {
  ...
  if (isOwner || isAdmin) {
    html += `
      <div class="channel-edit-fields">
        ...
        <label class="checkbox-row"><input type="checkbox" id="channelEditGroupCalls" ${conv.groupCallsEnabled !== false ? 'checked' : ''}> Разрешить групповые звонки</label>
  ...
  if (saveBtn) saveBtn.addEventListener('click', async () => {
    ...
    if (!name) return;
    const { conversation } = await api(`/api/conversations/${convId}`, { method: 'PATCH', body: { name, groupCallsEnabled, avatar: channelEditAvatarUrl } });
    mergeConversation(conversation);
...
function updateGroupCallButton(conv) {
  ...
  if (!conv || (conv.type !== 'channel' && conv.type !== 'group') || conv.groupCallsEnabled === false) {
